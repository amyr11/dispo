...
function PublicStatCard({ icon, label, value }: PublicStatCardProps) {
  return (
...
export default async function EventGalleryPage({
  ...
  const now = new Date()
  const { event, photos } = await eventsService.getPublicEventGallery(eventIdNum).catch((error) => {
    ...
    <div className="flex min-h-svh justify-center bg-muted">
      <div className="my-20 flex w-full max-w-lg flex-col px-4 sm:max-w-2xl">
        ...
        <section className="grid grid-cols-2 gap-3 py-8">
          <PublicStatCard icon={UserGroup02Icon} label="Attendees" value={stats.attendeesCount} />
          <PublicStatCard icon={Camera01Icon} label="Shots taken" value={stats.shotsCount} />
        </section>
